...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/02/09 19:40:17 by mbooth            #+#    #+#             */
/*   Updated: 2015/02/09 19:40:31 by mbooth           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void  continue_decimal_to_hex(size_t n, t_format *format, t_conversion *conversion, char *hex_str)
{
	char	fill;
	size_t	i;
	int		rem;
	...
	while (n != 0)
	{
		rem = n % 16;
		if (rem == 10)
			hex_str[i] = (conversion->specifier == HEX_UPPER ? 'A' : 'a');
		else if (rem == 11)
			hex_str[i] = (conversion->specifier == HEX_UPPER ? 'B' : 'b');
		else if (rem == 12)
			hex_str[i] = (conversion->specifier == HEX_UPPER ? 'C' : 'c');
		else if (rem == 13)
			hex_str[i] = (conversion->specifier == HEX_UPPER ? 'D' : 'd');
		else if (rem == 14)
			hex_str[i] = (conversion->specifier == HEX_UPPER ? 'E' : 'e');
		else if (rem == 15)
			hex_str[i] = (conversion->specifier == HEX_UPPER ? 'F' : 'f');
		else
			hex_str[i]= rem + '0';
		++i;
		n /= 16;
